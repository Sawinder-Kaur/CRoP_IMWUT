...
#     train_start, train_end, train_energy, train_memory_info,avg_percent  = train(model,best_model, train_loader, num_epochs, criterion, device, user_id, 'final')
    
#     print_training_stats(train_end-train_start, train_energy, train_memory_info,avg_percent,'Training')
    ...
    # model.load_state_dict(torch.load("global_model_bilstm.pt"),map_location=device)
    # global_model.load_state_dict(torch.load("global_model_bilstm.pt"),map_location=device)
    ...
    # for i in range(0,5):
        ...
        # trials.append([train_end-train_start, train_energy, train_memory_info,avg_percent])
    
    # avg=np.mean(trials,axis=0)
    
    # print_training_stats(avg[0],avg[1],avg[2],avg[3],'Generic Inference')
    ...
    avg=np.mean(trials,axis=0)
    print_training_stats(avg[0],avg[1],avg[2],avg[3],'Finetuned model Inference')
